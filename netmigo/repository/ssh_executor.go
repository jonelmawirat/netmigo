...
func ExecutorInteractiveExecute(client *ssh.Client, logger *slog.Logger, command string, timeoutSeconds int) (string, error) {
    ...
    logger.Info("Command execution complete", "outputFile", tempFile.Name())
    ...
    COLLECT_LOOP:
        for collecting {
            ...
            case errReader, ok := <-errorChannel:
                ...
                if !cmdOutputTimer.Stop() {
                    select { case <-cmdOutputTimer.C: default: }
                }
        ...
        tempFile, err := os.CreateTemp("", "cmd_multi_output_*.txt")
        if err != nil {
            logger.Error("Failed to create temp file for multiple command output", "command", cmd, "error", err)

            return outputFiles, fmt.Errorf("failed to create temp file for %q: %w", cmd, err)
        }
        if _, err := tempFile.WriteString(currentCmdOutput); err != nil {
            logger.Error("Failed to write to temp file for multiple command output", "command", cmd, "error", err)
            tempFile.Close()
            return outputFiles, fmt.Errorf("failed to write to temp file for %q: %w", cmd, err)
        }
        tempFile.Close()
        logger.Info("Command output saved (multiple)", "command", cmd, "file", tempFile.Name(), "sentinelSeen", sentinelSeenInOutput)
